...
// ---- Proxy Pool Manager (injected into config page) ----

function injectProxyManager() {
  const container = byId('config-container');
  if (!container || byId('proxy-manager-panel')) return;

  const panel = document.createElement('div');
  panel.id = 'proxy-manager-panel';
  ...
  panel.innerHTML = `
    ...
    <div class="flex items-center gap-3 mb-4">
      ...
      <button onclick="clearProxyPool()" class="geist-button-outline gap-2" style="border-color:#ef4444;color:#ef4444">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>
        <span>清空</span>
      </button>
      <span id="proxy-stats" class="text-sm text-[var(--accents-4)]"></span>
    </div>
    ...
    <div id="proxy-list-container" class="space-y-2">
      <div class="text-center py-4 text-[var(--accents-4)] text-sm">加载中...</div>
    </div>
  `;
...
async function clearProxyPool() {
  ...
  try {
    const res = await fetch('/v1/admin/proxies', { method: 'DELETE', headers: buildAuthHeaders(apiKey) });
    if (res.ok) {
